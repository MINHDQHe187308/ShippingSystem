...
(function ($) {
    $.fn.sortableLists = function (options) {
        ...
        var jQBody = $('body').css('position', 'relative'),
            ...
            opener = $('')//$('<span />') comment by thanhlv
...
function menuEditor(idSelector, settings) {
    ...
    $('#btnSubmitFrm').on('click', function () {
        var obj = inst.sortableListsToJson();
        var str = JSON.stringify(obj);
        $("#out_menu").text(str);
        // submit form when click save button
        $('#editMenu').submit();
    });
    //
    ...
    // edit
    $("#btnAddRole,#btnAddRole-page").click(function (e) {
        e.preventDefault();
        // check & add item selected, pass parametter with table id
        var tblID = $(this).attr('data-tblID');
        console.log(tblID);
        addItemMultiple(tblID);
        // reset list menu left 
        $(".checkSingle").each(function () {
            this.checked = false;
            $(this).parents('tr').removeClass("row-selected").css({ 'background-color': '#fff', 'color': '#363636' });
        });
        $("#" + tblID + " .checkedAll").prop("checked", false);
        $("#" + tblID + " .checkedAll").parents('tr').removeClass("row-selected").css({ 'background-color': '#fff', 'color': '#363636' });
        $(".panel table tr").removeClass("row-selected").css({ 'background-color': '#fff', 'color': '#363636' });
        //
    });
    /**
     * right menu - col-6
    ...
    */
    ...
    // form Edit menu by ThanhLv
    $(document).on('click', '.btnFrmEdit', function (e) {
        e.preventDefault();
        itemEdit = $(this).closest('li');
        $(this).closest('li').addClass("li-active");
        editItemMultiple(this);

    });
    // customize by ThanhLv
    function editItemMultiple(item) {
        // set data in form
        var dataObj = {
            id: $(item).closest('li').attr("data-id"),
            thumbnail: $(item).closest('li').attr("data-thumbnail"),
            excerpt: $(item).closest('li').attr("data-excerpt"),
            languagekey: $(item).closest('li').attr("data-languagekey"),
            name: $(item).closest('li').attr("data-name"),
            url: $(item).closest('li').attr("data-url"),
            target: $(item).closest('li').attr("data-target")
        };
        //
        var strModal = '';
        //<!-- Modal -->
        strModal += '<div class="modal fade" id="myModal"role = "dialog" tabindex="-1">';
        strModal += '<div class="modal-dialog modal-lg">';
        // <!-- Modal content-->
        strModal += '<div class="modal-content">';
        strModal += '<div class="panel panel-primary p-3" style="margin-bottom:0px;">';
        strModal += '<div class="panel-body">';
        strModal += '<form id="frmEdit" class="form-horizontal">';
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_id" class="col-sm-12 control-label">ID</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<input type="text" value="' + dataObj.id + '" class="form-control" id="mnu_id" name="mnu_id" placeholder="Text" readonly="readonly">';
        strModal += '</div>';
        strModal += '</div>';
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_name" class="col-sm-12 control-label">Tên menu</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<input type="text" value="' + dataObj.name + '" class="form-control" id="mnu_name" name="mnu_name" placeholder="Text">';
        strModal += '</div>';
        strModal += '</div>';
        // add excerpt
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_excerpt" class="col-sm-12 control-label">Mô tả ngắn</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<textarea class="form-control" id="mnu_excerpt" name="mnu_excerpt" placeholder="Excerpt ..." rows="4" cols="50">' + dataObj.excerpt + '</textarea>';
        strModal += '</div>';
        strModal += '</div>';
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_thumbnail" class="col-sm-12 control-label">Ảnh đại diện</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<input type="text" value="' + dataObj.thumbnail + '" class="form-control" id="mnu_thumbnail" name="mnu_thumbnail" placeholder="thumbnail">';
        strModal += '</div>';
        strModal += '</div>';
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_url" class="col-sm-12 control-label">URL</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<input type="text" value="' + dataObj.url + '" class="form-control" id="mnu_url" name="mnu_url" placeholder="URL">';
        strModal += '</div>';
        strModal += '</div>';
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_target" class="col-sm-12 control-label">Target</label>';
        strModal += '<div class="col-sm-12">';
        strModal += '<select id="mnu_target" name="mnu_target" class="form-control">';
        if (dataObj.target == "_blank") {
            strModal += '<option value="_self">Mặc định</option>';
            strModal += '<option value="_blank" selected>Mở tab mới</option>';
        } else {
            strModal += '<option value="_self" selected>Mặc định</option>';
            strModal += '<option value="_blank">Mở tab mới</option>';
        }
        strModal += '</select>';
        //
        strModal += '</div>';
        ...
        //
        strModal += '<div class="form-group">';
        strModal += '<label for="mnu_languagekey" class="col-sm-12 control-label">Language key</label>';
        strModal += '<div class="col-sm-12">';

        //strModal += '<input type="text" class="form-control" value="' + dataObj.languagekey + '" id="mnu_languagekey" name="mnu_languagekey" placeholder="Text">';
        strModal += '<select id="mnu_languagekey" name="mnu_languagekey" class="form-control">';
        if (dataObj.languagekey == "0") {
            strModal += '<option value="0" selected>Mặc định</option>';
            strModal += '<option value="1">Dropdown Collapse</option>';
            strModal += '<option value="2">Dropdown Horizontal</option>';
        } else if (dataObj.languagekey == "1") {
            strModal += '<option value="0">Mặc định</option>';
            strModal += '<option value="1" selected>Dropdown Collapse</option>';
            strModal += '<option value="2" >Dropdown Horizontal</option>';
        } else {
            strModal += '<option value="0">Mặc định</option>';
            strModal += '<option value="1" >Dropdown Collapse</option>';
            strModal += '<option value="2" selected>Dropdown Horizontal</option>';
        }
        strModal += '</select>';
        ...
        strModal += '<button type="button" id="btnCancel" class="btn btn-warning" data-dismiss="modal" aria-hidden="true" autocomplete="off">Hủy</button>';
        strModal += '<button type="button" id="btnSave" class="btn btn-success">Lưu</button>';
        strModal += '</div>';
        strModal += '</div>';
    ...
    // update data after close form edit. Create by ThanhLv
    function setDataItem() {
        $("#btnSave").click(function () {
            // get data attribute
            var mnuID = $("#mnu_id").val();
            var mnuName = $("#mnu_name").val();
            var mnuExcerpt = $("#mnu_excerpt").val();
            var mnuThumbnail = $("#mnu_thumbnail").val();
            var mnuUrl = $("#mnu_url").val();
            var mnuTarget = $("#mnu_target").val();
            var mnuLanguagekey = $("#mnu_languagekey").val();
            // set value 
            if (itemEdit === 0) {
                return;
            }
            // set data li
            itemEdit.find('span.txt').first().text(mnuName);
            $('#myList li.li-active').attr('data-id', mnuID);
            $('#myList li.li-active').attr('data-name', mnuName);
            $('#myList li.li-active').attr('data-excerpt', mnuExcerpt);
            $('#myList li.li-active').attr('data-thumbnail', mnuThumbnail);
            $('#myList li.li-active').attr('data-url', mnuUrl);
            $('#myList li.li-active').attr('data-target', mnuTarget);
            $('#myList li.li-active').attr('data-languagekey', mnuLanguagekey);
            //
            // hidden modal
            $(".list-group-item").removeClass("li-active");
            $("body").removeClass("modal-open");
    ...
    // customize by ThanhLv: c
    function addItemMultiple(tblID) {
        //exampleRight-page
        $("#" + tblID + " tbody .row-selected").each(function (index, element) {
            // get data attribute
            var mnuID = $(this).find($("div")).first().attr("data-id");
            var mnuName = $(this).find($("div")).first().attr("data-name");
            var mnuExcerpt = $(this).find($("div")).first().attr("data-excerpt");
            var mnuThumbnail = $(this).find($("div")).first().attr("data-thumbnail");
            var mnuUrl = $(this).find($("div")).first().attr("data-url");
            var mnuTarget = $(this).find($("div")).first().attr("data-target");
            var mnuLanguagekey = $(this).find($("div")).first().attr("data-languagekey");
            //
            var strItem = '';
            strItem += '<div class="group-item" style="position: relative;">';
            strItem += '<div class="item-menu" style="display: inline-block">';
            strItem += '<span class="txt" style="padding-right: 5px;">' + mnuName + '</span>';
            strItem += '</div>';
            //
            strItem += '<div class="btn-group pull-right" style="position: absolute; top: 50%; right: 0px;transform: translate(0, -50%);">';
            strItem += '<a href="" class="btn-act btn btn-default btn-xs btnFrmEdit">' + labelEdit + '</a> ';
            strItem += '<a href="" class="btn-act btn btn-danger btn-xs btnRemove">' + labelRemove + '</a>';
            strItem += '</div>';
            //
            strItem += '</div>';
            //
            var li = $("<li>");
            li.attr({ "data-id": mnuID, "data-excerpt": mnuExcerpt, "data-thumbnail": mnuThumbnail, "data-url": mnuUrl, "data-name": mnuName, "data-target": mnuTarget, "data-languagekey": mnuLanguagekey });
            li.addClass('list-group-item').css({ "padding": "5px", "cursor": "move" }).append(strItem);
            // add li vao menu
            $('#' + idSelector).append(li);
        });     // endEach;
    }
    ...
    function addAddRowItem() {
        // get data attribute
        var mnuID = "0";
        var mnuThumbnail = "";
        var mnuExcerpt = "";
        var mnuName = "Text Menu";
        var mnuUrl = "#";
        var mnuTarget = "_self";
        var mnuLanguagekey = "mnu_";
        //
        var strItem = '';
        strItem += '<div class="group-item" style="position: relative;">';
        strItem += '<div class="item-menu" style="display: inline-block">';
    ...
    * */
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>');
            $li.attr({ 'id': v.text, 'data-id': v.id, 'data-excerpt': v.excerpt, 'data-thumbnail': v.thumbnail, 'data-name': v.name, 'data-url': v.url, 'data-target': v.target, 'data-languagekey': v.languagekey });
            $li.addClass('list-group-item').data('text', v.text).data('icon', v.icon).data('href', v.href);
            $li.css({ "padding": "5px", "cursor": "move" });
            // div.group-item
            var $div = $('<div>');
            $div.addClass("group-item");
            $div.css({ 'position': 'relative', });
            //div.item-menu
            var $i = $('<div>').addClass('item-menu ');
            $i.css({ 'display': 'inline-block' });
            //span.txt
            var $span = $('<span>').addClass('txt').css({ 'padding-right': '5px' }).append(v.name);
            //$span nam trong $i = div.item-menu
            $i.append($span);
            //
            var $divbtn = $('<div>').addClass('btn-group pull-right');
            $divbtn.css({ 'position': 'absolute', 'top': '50%', 'right': '0px', 'transform': 'translate(0, -50%)' });
            //
            var $btnEdit = TButton({ classCss: 'btn-act btn btn-default btn-xs btnFrmEdit', text: labelEdit });
            var $btnRemv = TButton({ classCss: 'btn-act btn btn-danger btn-xs btnRemove', text: labelRemove });
            //
            $divbtn.append($btnEdit).append($btnRemv);
            $div.append($i).append("&nbsp;").append($divbtn);
            $li.append($div);
            if (isParent) {
